...
{
    //是否使用初始值，useInitialGuess默认为false
    struct timeval tv_start, tv_end;
    struct timeval tv_start0, tv_end0;
    double time_load =0, time_match=0, time_combined=0;

    // gettimeofday(&tv_start0,NULL); //总体执行时间的起始点

    //initial guess
    pcl::PointCloud<pcl::PointXYZ> sourceCloud = source;
    Eigen::Transform<double,3,Eigen::Affine,Eigen::ColMajor> Temp, Tinit; // 建立变换矩阵
    Tinit.setIdentity();
    if(useInitialGuess) // 是否使用初始矩阵
    {
...
#endif
    {
        ...
        for(int r_ctr = resolutions.size()-1; r_ctr >=0;  r_ctr--) //迭代循环resolutions中的格网尺寸
        {

            current_resolution = resolutions[r_ctr]; //当前的格网尺寸
            
            gettimeofday(&tv_start0,NULL); //总体执行时间的起始点

            LazyGrid prototypeSource(current_resolution);
            LazyGrid prototypeTarget(current_resolution); // 建立格网

            gettimeofday(&tv_start,NULL);
            NDTMap targetNDT( &prototypeTarget ); // 输入一个空间索引，建立NDT
            targetNDT.loadPointCloud( target ); // 加载目标点云
            targetNDT.computeNDTCells(); // 参考点云建立NDT

            NDTMap sourceNDT( &prototypeSource );
            sourceNDT.loadPointCloud( sourceCloud );
            sourceNDT.computeNDTCells();// 源点云建立NDT
            gettimeofday(&tv_end,NULL);
            // tv_sec是秒，tv_usec是微秒，下面运算统一转换为毫秒ms
            time_load += (tv_end.tv_sec-tv_start.tv_sec)*1000.+(tv_end.tv_usec-tv_start.tv_usec)/1000.; // 目标点云与源点云建立NDT时间
            Temp.setIdentity();

            gettimeofday(&tv_start,NULL);
            ret = this->match( targetNDT, sourceNDT, Temp ); //循环使用match，注意参数变成了NDT,所以使用的match函数也不一样
            lslgeneric::transformPointCloudInPlace(Temp,sourceCloud); // 使用临时的T来变换源点云
            gettimeofday(&tv_end,NULL);

            time_match += (tv_end.tv_sec-tv_start.tv_sec)*1000.+(tv_end.tv_usec-tv_start.tv_usec)/1000.; // 两个NDT进行匹配使用的时间

            //transform moving
            T = Temp*T; //ORIGINAL  // 修正变换矩阵
            //T = T*Temp;

            gettimeofday(&tv_end0,NULL);
            time_combined = (tv_end0.tv_sec-tv_start0.tv_sec)*1000.+(tv_end0.tv_usec-tv_start0.tv_usec)/1000.; // 建立NDT过程与匹配过程两者时间之和
            //std::cout << "resolution:" << resolutions[r_ctr] << ", "<<"load: "<<time_load<<" match "<<time_match<<" combined "<<time_combined<< " the best score:" << this->finalscore << std::endl;

            //std::cout << "the best score:" << this->finalscore << std::endl;
            time_load =0; time_match=0; time_combined=0; //恢复为默认值
...
bool NDTMatcherD2D::match( NDTMap& targetNDT,
        NDTMap& sourceNDT,
        Eigen::Transform<double,3,Eigen::Affine,Eigen::ColMajor>& T ,
        bool useInitialGuess) // 注意参数为 NDT,所以使用的match函数内容与之前的不同
{

    //locals
    bool convergence = false;
    //double score=0;
    double score_best = INT_MAX; // score_best先定义为INT_MAX，最大为2147483647
...
//	std::cout<<"itr "<<itr_ctr<<std::endl;
	if(score_here < score_best) // 获取最小的score作为最优值
	{
	    Tbest = T;
	    score_best = score_here; // 获取最小的score作为最优值
...
//    std::cout<<"grad(:,"<<itr_ctr+1<<") = [0 0 0 0 0 0]';\n";
    ...
    this->finalscore = score_best/NUMBER_OF_ACTIVE_CELLS;
    //std::cout<< "my_score " << this->finalscore << "my_score_best " << score_best << "NUMBER_cell" << NUMBER_OF_ACTIVE_CELLS << std::endl;
    return ret;
}
