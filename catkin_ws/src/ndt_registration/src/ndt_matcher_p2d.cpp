...
#include <sys/time.h> //xiaochen记录时间
        ...
        Eigen::Transform<double,3,Eigen::Affine,Eigen::ColMajor>& T )
{
    ...
    double time_load =0, time_match=0, time_combined=0; // 定义时间变量
    ...
    gettimeofday(&tv_start,NULL); //预处理，目标点云建立NDT 起始点
	LazyGrid prototype(current_resolution);
	NDTMap ndt( &prototype );
	ndt.loadPointCloud( target );
	ndt.computeNDTCells();
    gettimeofday(&tv_end,NULL); //预处理，目标点云建立NDT 结束点
    time_load += (tv_end.tv_sec-tv_start.tv_sec)*1000.+(tv_end.tv_usec-tv_start.tv_usec)/1000.; // 目标点云建立NDT时间

	gettimeofday(&tv_start,NULL);// 获取转换参数，进行匹配 起始点
    ret = this->match( ndt, moving, Temp );
	T = Temp*T;
	//transform moving
	lslgeneric::transformPointCloudInPlace(Temp,moving);
    gettimeofday(&tv_end,NULL); // 获取转换参数，进行匹配 结束点
    time_match += (tv_end.tv_sec-tv_start.tv_sec)*1000.+(tv_end.tv_usec-tv_start.tv_usec)/1000.; // 进行匹配时间
    ...
    // std::cout << "resolution:" << resolutions[i] << ", "<<"load: "<<time_load<<" match "<<time_match<<" combined "<<time_combined<< " the best score:" << this->finalscore << std::endl;
